...
describe('previousPrimeArray()', function () {
  it('should return a new array where each prime is replaced with the prime that comes before it', function () {
    assert.deepEqual(previousPrimeArray([10, 12, 11, 7, 16]), [ 10, 12, 7, 5, 16 ]);
    assert.deepEqual(previousPrimeArray([17, 24, 29, 5, 2, 4]), [ 13, 24, 23, 3, null, 4 ]);
    assert.deepEqual(previousPrimeArray([1, 2, 3, 4, 5]), [ 1, null, 2, 4, 3 ]);
  });
});
