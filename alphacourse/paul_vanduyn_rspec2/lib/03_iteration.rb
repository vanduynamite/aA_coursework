...
def factors(num)
  (1..num/2).collect {|i| i if num % i == 0}.compact << num
end
...
def substrings(string)
  ...
  (0...string.length).each do |i|
    ...
    (i...string.length).each do |j|
      ...
      array << string[i..j] unless array.include?(string[i..j])
...
def subwords(word, dictionary)
  substrings(word).select {|s| dictionary.include?(s)}
end
...
class Array
  ...
  def my_inject(&blk)
    ...
    ## this one doesn't work because val is being set to nil by the block
    # val = nil
    #
    # self.my_each do |el|
    #   if val
...
def concatenate(strings)
  strings.inject('') {|conc, str| conc + str}
end
