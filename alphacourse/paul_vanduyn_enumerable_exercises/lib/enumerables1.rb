...
def array_sum_with_index(arr)
  ...
  arr.each_with_index.reduce(0) {|acc, pair| acc + pair[0] * pair[1]}
...
def at_least_n_factors(numbers, n)
  ...
  numbers.select {|i| num_factors(i) >= n}
...
def ordered_vowel_word?(word)
  ...
  vowel_indices = word.chars.map {|char| 'aeiou'.chars.index(char)}.compact

  vowel_indices.sort == vowel_indices
...
def products_except_me(numbers)
  ...
  numbers.map {|el| numbers.reduce(:*) / el}
