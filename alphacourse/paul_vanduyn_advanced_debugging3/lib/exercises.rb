class Array
  ...
  def dups
    ...
    self.each_with_index do |item, index|
      positions[item].nil? ? positions[item] = [index] : positions[item] << index
    end
...
class Array
  ...
  def pair_sum(target)
    ...
    self.each_with_index do |el1, i|
      self.drop(i).each_with_index do |el2, j|
        if el1 + el2 == target
          pairs << [i, j+i] unless el1 == 0 && el2 == 0
        end
      end
    end
    pairs
...
def transpose(matrix)
  result = Array.new(matrix[0].length).map! {[]} #HARD - PASS BY REFERENCE ISSUE
